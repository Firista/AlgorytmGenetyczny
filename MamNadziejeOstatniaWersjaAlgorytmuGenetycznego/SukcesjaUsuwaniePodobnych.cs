...
{
    public class SukcesjaUsuwaniePodobnych
    {
        ...
        public List<Osobnik> usunPodobnychOsobnikow(List<Osobnik> nowePokolenie)
        {
            List<Osobnik> nowaPopulacjaDoModyfikacji = nowePokolenie;

            double wartoscMinimalna = 0;
            double wartoscMaxymalna = 0;
            double wartoscBledu = 0.0005;
            Osobnik Wybraniec;
            Osobnik doWyrzucenia;

            var rng = new Random(Guid.NewGuid().GetHashCode());

            for (int i = 0; i < nowaPopulacjaDoModyfikacji.Count; i++)
            {
                Wybraniec = nowaPopulacjaDoModyfikacji[i];
                wartoscMinimalna = (Wybraniec.wartoscOsobnika - wartoscBledu);
                wartoscMaxymalna = (Wybraniec.wartoscOsobnika + wartoscBledu);

                for (int j = 0; j < nowaPopulacjaDoModyfikacji.Count; j++)
                {
                    if (j != i)
                    {
                        if ((nowaPopulacjaDoModyfikacji[j].wartoscOsobnika >= wartoscMinimalna) && (nowaPopulacjaDoModyfikacji[j].wartoscOsobnika <= wartoscMaxymalna))
                        {
                            doWyrzucenia = nowaPopulacjaDoModyfikacji[j];

                            if (doWyrzucenia.Equals(Wybraniec))
                            {
                                nowaPopulacjaDoModyfikacji = wyrzucanieOsobnika(doWyrzucenia, nowaPopulacjaDoModyfikacji);
                                nowaPopulacjaDoModyfikacji.Add(Wybraniec);
                            }
                            else
                            {
                                nowaPopulacjaDoModyfikacji = wyrzucanieOsobnika(doWyrzucenia, nowaPopulacjaDoModyfikacji);
            ...
            return nowaPopulacjaDoModyfikacji;
        }
